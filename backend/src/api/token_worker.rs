...
pub fn start_token_worker(state: AppState) {
    tokio::spawn(async move {
        ...
        loop {
            ...
            tracing::info!("Found {} accounts requiring token refresh", expiring_accounts.len());
            ...
            for mut account in expiring_accounts {
                ...
                match res {
                                ...
                                match &state.db {
                                    DbPool::Sqlite(pool) => {
                                        ...
                                        if let Err(e) = update_res {
                                            tracing::error!("Failed to update refreshed token for account {}: {}", account.id.0, e);
                                        } else {
                                            tracing::info!("Successfully refreshed token for account {}", account.id.0);
                                        }
                                    },
                                    DbPool::Postgres(pool) => {
                    ...
                    Ok(resp) => {
                        ...
                        tracing::error!("Token refresh failed for account {} ({}): {}", account.id.0, status, body);
                    },
                    Err(e) => {
                        tracing::error!("Network error refreshing token for account {}: {}", account.id.0, e);
                    }
                }
            }
        }
